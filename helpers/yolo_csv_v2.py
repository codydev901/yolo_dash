...
class YoloCSVTwo:
    ...
    def _handle_step_info(step_info, graph_dict, graph_helper):
        ...
        # print(graph_helper)
        # print(list(graph_helper["pred_id_last_node"].keys()))
        ...
        for step in step_info:
            ...
            step_arr = [step[k] for k in step]
            # rint(step_arr)
            if int(step["predecessorID"]) not in graph_helper["pred_id_last_node"]:
                last_pred_id = max(int(step["predecessorID"]) - 1, 1)
                branch_node_name = "{}.{}".format(step["time_num"], last_pred_id)
                ...
                # print("GOT BRANCH:", branch_node_name, step_arr, "NextPredID:{}".format(step["predecessorID"]))
                graph_helper["pred_id_last_node"][step["predecessorID"]] = branch_node_name
                active_pred_ids += [step["predecessorID"]]
        ...
        for k in list(graph_helper["pred_id_last_node"].keys()):
            if k not in active_pred_ids:
                del graph_helper["pred_id_last_node"][k]
